...
class ImportanceData():
	...
	def measure_correlation(self):
			...
			for entry_nr in range(len(covar_matrix[row_nr])):
				if row_nr > entry_nr:
					if abs(covar_matrix[row_nr][entry_nr]) \
						...
						strong_corr_inputs\
							.append([[str(row_nr), str(entry_nr)], \
		...
		if len(strong_corr_inputs) > 0 or len(weak_corr_inputs) > 0:
			print("Caution!\n" 
				+ "Correlation between input data can affect the reliability "
				+ "of the importance measure.\n"
				+ "Strong correlations of more than {} "\
					.format(self._strong_corr_threshold) 
				+ "were found between {} pair(s) "
					.format(len(strong_corr_inputs))
				+ "of input variables:\n\t"
				+ "\n\t".join([": ".join([",".join(subentry) \
					if isinstance(subentry, list) else subentry \
					for subentry in entry]) for entry in strong_corr_inputs])
				+ "\nAdditionally, weak correlations of more than " 
				+ "{} were found between ".format(self._weak_corr_threshold) 
				+ "{} pair(s) of input variables:\n\t"\
					.format(len(weak_corr_inputs)) 
				+ "\n\t".join([": ".join([",".join(subentry) \
					if isinstance(subentry, list) else subentry \
					for subentry in entry]) for entry in weak_corr_inputs]))
		else:
	...
	def perturb_snapshots(self, mod_along, perturbation):
		...
		rand_array = np.random.uniform(1 - perturbation, 1 + perturbation,
			self._snapshot_cnt)
	...
	def HIPR_snapshots(self, mod_along, min_value, max_value):
		transposed = deepcopy(self._columns)
		rand_array = np.random.uniform(min_value, max_value,
			self._snapshot_cnt)
	...
	def plot_super_importance(
		...
		# the batch size
		batch_size = np.shape(list(val_ds\
			.as_numpy_iterator())[0][0][model.input_names[0]])[0]
		fig, axs = plt.subplots(len(loss_names), len(modes), \
			figsize=(subfig_size*len(modes),subfig_size*len(loss_names)))
		fig.suptitle(
