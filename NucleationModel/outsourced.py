shooting_points, shooting_labels = read_shooting_points("total_data_till_982mc_280K.txt")
...
def make_xy_linspaces(pipeline, x_int, y_int):
    xs = np.linspace(pipeline.lower_bound[x_int], pipeline.upper_bound[x_int], 11)
    ys = np.array([68.14 -0.4286*x for x in xs])
    xs = (xs - pipeline.mean[0]) / pipeline.std[0]
    ys = (ys - pipeline.mean[1]) / pipeline.std[1]
    xs = (xs - pipeline.minima[0]) / (pipeline.maxima[0] - pipeline.minima[0]) * c.resolution
    ys = (ys - pipeline.minima[1]) / (pipeline.maxima[1] - pipeline.minima[1]) * c.resolution
...
def compare_dividing_surfaces():
    ...
    plot_one_map(
        train_label_map, 
        "DividingSurfaceTest_trainData_"\
            + c.data_stamp)
    plot_one_map(
        shooting_label_map, 
        "DividingSurfaceTest_shootingData_"\
            + c.data_stamp)
                ...
                for i in range(len(map1[y][x][z][z])):
                    for j in range(len(map1[y][x][z][z][i])):
                        if map1[y][x][z][z][i][j] != map2[y][x][z][z][i][j] \
                            ...
                            are_equal = False
                            unequal_counter += 1
                            if map1[y][x][z][z][i][j] - map2[y][x][z][z][i][j] > 0.01:
                                above_counter += 1
