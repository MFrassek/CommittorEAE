...
class pB_Balancer():
	...
	def balance_and_mask(pBs, bins):
		...
		round_pBs = np.ceil((np.array(pBs) * (bins + 1)))
		counter = Counter(round_pBs)
		masked_balanced_counter = {key: pBs_len/label if key != 0 and \
			key != bins + 1 else 0 for key, label in counter.items()}
		masked_pB_balanced_weights = np.array([masked_balanced_counter[i] \
			for i in round_pBs])
	...
	def trim_and_balance(pBs, bins):
		...
		trimmed_balanced_counter = {key: trimmed_pBs_len/label for key, label \
			in counter.items()}
		trimmed_pB_balanced_weights = np.array([trimmed_balanced_counter[i] \
			for i in trimmed_round_pBs])
